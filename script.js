...
window.addEventListener("load", () => {
    ...
    if (PHP_ASSIGNED_PUZZLE && PHP_ASSIGNED_DIFF !== null) {
        ...
        // Show a brief "Your puzzle:" message then auto-start after 1.5s
        ...
        // Don't run the rest of init that overwrites difficulty
        if (startLevelInput) startLevelInput.disabled = true;

    } else {
        // Normal (non-assigned) load
        if (startLevelInput) {
...
async function flashSequence() {
    ...
    // Skip button — sits in controls bar between ← Puzzles and Give Up
...
function startShapePuzzle() {
    ...
    // Skip preview button in controls bar
    document.getElementById("skip-flash-btn")?.remove();
...
function showLobbyUI() {
    const content = `
        <div style="background:rgba(0,0,0,.3);padding:18px;border-radius:12px;margin-bottom:14px;">
            ...
            <h1 style="color:var(--magenta);font-size:2.4rem;letter-spacing:8px;margin:0;font-family:monospace;cursor:pointer;" onclick="navigator.clipboard.writeText('${roomCode}');this.textContent='Copied!';setTimeout(()=>this.textContent='${roomCode}',1500);">${roomCode}</h1>
        </div>
    ...
    document.getElementById("cancel-btn")?.addEventListener("click", () => { clearInterval(syncTimer); hideMPOverlay(); roomCode=null; isHost=false; gameStarted=false; });
}
async function markReady() {
    playerReady = true;
    const fd = new FormData(); fd.append("code", roomCode); fd.append("role", isHost?"host":"guest");
    await fetch("php/ready.php", { method:"POST", body:fd });
}
function startPolling() { clearInterval(syncTimer); syncTimer = setInterval(pollRoom, 1500); }
async function pollRoom() {
    try {
        const res = await fetch(`php/poll_room.php?code=${roomCode}`);
        const data = await res.json();
